...
def dict_to_schema(param_dict: Dict[str, Any]) -> 'Schema':
    ...
    # Handle empty or None param_dict
    if not param_dict:
        # Return minimal object schema
        schema.type_ = Type.OBJECT
        return schema
...
def convert_to_genai_function_declarations(tools: Sequence[Union[_ToolDict, GoogleTool]]) -> GoogleTool:
    ...
    for tool in tools:
        if isinstance(tool, GoogleTool):
            # If it's already a GoogleTool, extract its function declarations
            ...
            if "function" in tool:
                ...
                if not parameters_dict or parameters_dict == {}:
                    parameters_dict = {
                        "type": "object",
                        "properties": {}
                    }
            ...
            for param_name, param in sig.parameters.items():
                ...
                # Extract description from annotation if available
...
# Replace the existing _chat_with_retry function with this one
...
def _chat_with_retry(generation_method: Callable, **kwargs: Any) -> Any:
    ...
    This version includes specific handling for tool-calling errors on custom endpoints.
    """
    retry_decorator = _create_retry_decorator()

    @retry_decorator
    def _execute_chat(**kwargs: Any) -> Any:
        try:
            # First attempt
            return generation_method(**kwargs)
        
        except google.api_core.exceptions.InvalidArgument as e:
            # This is the most common error for unsupported tool calls.
            # Check if it's a tool-related error and if we are using a custom endpoint.
            is_tool_error = "tools" in str(e).lower() or "function" in str(e).lower()
            has_custom_endpoint = "api_endpoint" in str(kwargs).lower()

            if is_tool_error and has_custom_endpoint:
                logger.warning(
                    f"Tool calling failed on custom endpoint with InvalidArgument: {e}. "
                    "Retrying without tools."
                )
                
                # Create a new, clean request without tools for the retry.
                request = kwargs.get("request")
                if request and (hasattr(request, 'tools') and request.tools):
                    # Create a new kwargs dict for the retry call
                    retry_kwargs = kwargs.copy()
                    
                    # Create a new request object from the old one, but without tools
                    from google.ai.generativelanguage_v1beta.types import GenerateContentRequest
                    new_request = GenerateContentRequest(
                        model=request.model,
                        contents=request.contents,
                        safety_settings=request.safety_settings,
                        generation_config=request.generation_config,
                        # EXPLICITLY OMITTING tools and tool_config
                    )
                    retry_kwargs['request'] = new_request
                    
                    # Make the second attempt without tools
                    return generation_method(**retry_kwargs)

            # If it's not a recoverable tool error, raise it
            raise ChatGoogleGenerativeAIError(
                f"Invalid argument provided to Gemini. Original error: {e}"
            ) from e

        except google.api_core.exceptions.FailedPrecondition as exc:
            if "location is not supported" in exc.message:
                raise ValueError("Your location is not supported by google-generativeai.")
            raise  # Re-raise other precondition failures

        except Exception as e:
            # Catch any other unexpected errors
            logger.error(f"An unexpected error occurred during the API call: {e}")
            raise ChatGoogleGenerativeAIError(
                f"An unexpected error occurred. Original error: {e}"
            ) from e

    # Initial call to the decorated function
    return _execute_chat(**kwargs)
...
#         except google.api_core.exceptions.InvalidArgument as e:
#             # Check if this is a tool-related error for custom endpoint
#             if "tools" in str(e).lower() or "function" in str(e).lower():
#                 logger.warning(f"Tool calling may not be supported on custom endpoint: {e}")
#                 # Try without tools by removing them from the request
...
#             # Custom endpoint sometimes returns 500 errors for tool calling
#             if "tools" in str(kwargs).lower():
#                 logger.warning(f"Internal server error with tools on custom endpoint: {e}")
#                 raise ChatGoogleGenerativeAIError(
#                     f"Custom endpoint returned internal server error: {e}"
#                 ) from e
#             raise ChatGoogleGenerativeAIError(
#                 f"Internal server error from custom endpoint: {e}"
#             ) from e
#         except Exception as e:
...
class ChatGoogleGenerativeAI_H(_BaseGoogleGenerativeAI, BaseChatModel):
    ...
    def validate_environment(self) -> Self:
        ...
        if google_api_key:
            # Add API key to metadata (custom endpoint may use x-api-key)
            metadata = list(self.default_metadata)
            metadata.append(("x-api-key", google_api_key))
        ...
        try:
            ...
            # If client creation fails with custom endpoint, try with different transport
            if client_options.get("api_endpoint") and transport == "rest":
                logger.warning(f"Failed to create client with REST transport: {e}. Trying GRPC...")
                try:
                    client_kwargs["transport"] = "grpc"
                    self.client = GenerativeServiceClient(**client_kwargs)
                except Exception as grpc_error:
                    logger.error(f"Failed to create client with both REST and GRPC transports. REST error: {e}, GRPC error: {grpc_error}")
                    raise ChatGoogleGenerativeAIError(
                        f"Failed to initialize Google Generative AI client for custom endpoint. "
                        f"Please check your API key and endpoint configuration. Original error: {e}"
                    ) from e
            else:
                raise ChatGoogleGenerativeAIError(
                    f"Failed to initialize Google Generative AI client: {e}"
                ) from e
